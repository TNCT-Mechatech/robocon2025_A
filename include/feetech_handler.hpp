...
class FeetechHandler
{
 public:
  ...
  bool SetTorqueEnable(int id, bool state)
  {
    if (!config_list_.count(id))
    {
      printf("id[%d] not found in config_list\n", id);
    ...
    // const ServoConfig config = config_list_.at(id);
    std::vector<unsigned char> send_data = GenerateSetTorqueEnableCommand(id, state);
    SendCommandAndWait(send_data);
    return true;
  }
 ...
 private:
  ...
  {
    uint16_t pos_uint16 = (pos < 0) ? static_cast<uint16_t>(0x8000 - pos) : static_cast<uint16_t>(pos);
    uint16_t vel_uint16 = (vel < 0) ? static_cast<uint16_t>(0x8000 - vel) : static_cast<uint16_t>(vel);
  ...
  static std::vector<unsigned char> GenerateSetTorqueEnableCommand(const int id, const bool state)
  {
    std::vector<unsigned char> buffer;
    buffer.push_back(id);
    buffer.push_back(4);     // len
